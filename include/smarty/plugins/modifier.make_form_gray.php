...
function smarty_modifier_make_form_gray($form_name,$form_setting,$error,$colspan = SMARTY_BOOL_OFF,$isConfirm = SMARTY_BOOL_OFF)
{
    global $con;
    ...
    $html = '';
    $html .= "<tr>\n";
    $html .= '<td class="form_ttl"><table cellspacing="0" cellpadding="5"><tr><td lang="en">';
    ...
    $html .= "</td></tr></table></td>\n";
    $html .= '<td lang="en" class="form_data">';
    ...
    for ($i=0;$i<$count;$i++){
        ...
        case 'text':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                //特殊処理
                if($setting[$i]['name'] == 'mail'){
                    $data = '<input lang="en" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' /><br class="clear" />';
                }elseif($setting[$i]['name'] == 'mail_m'){
                    $data = '<input lang="en" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' /><p class="p_fl">&nbsp;＠&nbsp;</p>';
                }else{
                    $data = '<input lang="en" type="text" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" class="'.$setting[$i]['class'].'"'.$maxlength.' />';
                }
        ...
        case 'password':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                $data = '<input lang="en" type="password" name="'.$setting[$i]['name'].'" value="" class="'.$setting[$i]['class'].'"'.$maxlength.' />';
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
            break;
        ...
        case 'textarea':
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                $data = '<textarea lang="en" name="'.$setting[$i]['name'].'" class="'.$setting[$i]['class'].'" />'.$escape_post.'</textarea>';
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
            break;
            ...
            if($isConfirm === SMARTY_BOOL_ON){
                ...
                foreach ($setting[$i]['value'] as $key => $value){
                    ...
                    if(strcasecmp($escape_post,$key) == 0){
                        ...
                        $checked = '';//デフォルトはチェックなし
                    }
                    $labe_id = $setting[$i]['name'].'_'.$i2;
                    $data .= '<input lang="en" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                    $i2++;
                }
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
...
/*                    elseif((strlen($escape_post) == 0 || !isset($escape_post)) && $i2 == 0){
                    ...
                    $labe_id = $setting[$i]['name'].'_'.$i2;
                    $data .= '<input lang="en" type="checkbox" id="'.$labe_id.'" name="'.$setting[$i]['name'].'[]" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                    $i2++;
                }
            }
            $html .= $setting[$i]['front'].$data.$setting[$i]['back'];
        ...
        case 'select'://セレクト：valueの数だけoptionが表示される.selectではないことに注意
            if($isConfirm === SMARTY_BOOL_ON){
                $data = $setting[$i]['value'][$escape_post];
                if($setting[$i]['name'] == 'number_of_persons' && $_POST['children'][0] == 1){
                    $data .= ' (Children)';
                    $data .= '<input lang="en" type="hidden" name="children[]" value="1" />';
                }
            }else{
                $data .= '<select lang="en" name="'.$setting[$i]['name'].'">'."\n";
                $data .= $setting[$i]['must'] ? '<option value="">---------</option>'."\n" : '<option value="">---------</option>'."\n";
                foreach ($setting[$i]['value'] as $key => $value){
                    $selected = '';
                ...
                $data .= '</select>'."\n";
                if($setting[$i]['name'] == 'number_of_persons'){
                    $children_check = isset($_POST['children']) && in_array(1,$_POST['children']) ? ' checked' : '';
                    $data .= '<input lang="en" type="checkbox" id="children_0" name="children[]" value="1" class="" '.$children_check.'  /><label for="children_0">Children</label>';
                }
            }
            
            if($count > 1) $html .= '<p class="m_b10">';//複数ある場合はpタグで囲む
            if($setting[$i]['front'] != '') $html .= '<span lang="en" class="m_r10">'.$setting[$i]['front'].'</span>';//前テキストがある場合はマージンをとる
            $html .= $data;
            $html .= $setting[$i]['back']."\n";
        ...
        if($isConfirm === SMARTY_BOOL_ON){
            if(!is_array($escape_post)){
                $html .= '<input lang="en" type="hidden" name="'.$setting[$i]['name'].'" value="'.$escape_post.'" />';//hidden
            }else{
                foreach ($escape_post as $key => $value){
                    $html .= '<input lang="en" type="hidden" name="'.$setting[$i]['name'].'[]" value="'.$value.'" />';//hidden
                }
            }
